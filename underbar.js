...
const map = function(obj, callback=identity) {
  let results = [];
  each(obj, (item, index, obj) => {
    results.push(callback(item, index, obj));
  });
  return results;
};
...
const reduce = function(obj, callback=identity, initialValue) {
  ...
  each(obj, (item, index, obj) => {
    if (initializing) {
      ...
      accumulator = callback(accumulator, item, index, obj);
    }
  });
  return accumulator;
...
const contains = function(obj, target) {
  return reduce(obj, (wasFound, currValue) => {
    return wasFound || currValue === target;
  }, false);
};
...
const uniq = function(obj) {
  ...
  return filter(obj, item => {
    return (foundItems[item] === undefined) && (foundItems[item] = true);
  });
};
